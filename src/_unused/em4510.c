...
char *addressing_modes[] = { "", "($nn,X)", "$nn", "#$nn", "A", "$nnnn", "$nn,$rr", "$rr", "($nn),Y", "($nn),Z", "$rrrr",
  "$nn,X", "$nnnn,Y", "$nnnn,X", "($nnnn)", "($nnnn,X)", "($nn,SP),Y", "$nn,Y", "#$nnnn", NULL };
...
int resolve_address_to_long(unsigned short short_address, int writeP, struct ctx4510 *cpu)
{
  ...
  // -- C64 KERNEL
  if reg_map_high (3)
    = '0' then if (blocknum = 14) and (lhc(1) = '1')
  and (writeP = false) then temp_address(27 downto 12)
        : = x "002E";
  end if;
  if (blocknum = 15)
    and(lhc(1) = '1')
  ...
  --C64 BASIC if reg_map_high (1) = '0' then if (blocknum = 10) and (lhc(0) = '1')
                                and (writeP = false) then temp_address(27 downto 12)
      : = x "002A";
  end if;
  if (blocknum = 11)
  ...
  --$D030 ROM select lines : if (blocknum = 14 or blocknum = 15) and rom_at_e000 = '1' then temp_address(27 downto 12)
      : = x "003E";
  if blocknum
    = 15 then temp_address(12)
        : = '1';
  end if;
  end if;
  if (blocknum = 12)
    and rom_at_c000 = '1' then temp_address(27 downto 12)
        : = x "002C";
  end if;
  if (blocknum = 10 or blocknum = 11)
    and rom_at_a000 = '1' then temp_address(27 downto 12)
        : = x "003A";
  if blocknum
    = 11 then temp_address(12)
        : = '1';
  end if;
  end if;
  if (blocknum = 9)
    and rom_at_9000 = '1' then temp_address(27 downto 12)
        : = x "0039";
  end if;
  if (blocknum = 8)
    and rom_at_8000 = '1' then temp_address(27 downto 12)
        : = x "0038";
  end if;

  --Hyppo ROM(takes precedence over all else if enabled) if (blocknum = 14) and (hyppo_en = '1')
      and (writeP = false) then temp_address(27 downto 12)
      : = x "FFFE";
  end if;
  if (blocknum = 15)
  ...
  int cpu_step(struct ctx4510 * cpu)
  {
    printf("PC=$%04x, A=$%02x, X=$%02x, Y=$%02x, Z=$%02x, B=$%02x, SP=$%04x, P=%c%c-%c%c%c%c%c\n", cpu->pc, cpu->a, cpu->x,
        cpu->y, cpu->z, cpu->b, cpu->sp, cpu->flag_n ? 'N' : 'n', cpu->flag_v ? 'V' : 'v', cpu->flag_e ? 'E' : 'e',
        cpu->flag_d ? 'D' : 'd', cpu->flag_i ? 'I' : 'i', cpu->flag_z ? 'Z' : 'z', cpu->flag_c ? 'C' : 'c');
