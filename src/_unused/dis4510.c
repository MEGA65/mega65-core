...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < bytes;) {
    ...
    if (annotation_address > 65536)
      annotation_address &= 0xffff;
    if (annotation_address < 0)
      annotation_address &= 0xffff;
    ...
    if (!is_data[load_address + i - 1]) {
      if (!strcasecmp(opnames[opcode], "RTI"))
        printf("\n");
      if (!strcasecmp(opnames[opcode], "RTS"))
        printf("\n");
      if (!strcasecmp(opnames[opcode], "JMP"))
        printf("\n");
      if (!strcasecmp(opnames[opcode], "BRA"))
        printf("\n");
    }
