...
int main(void)
{
  ...
  int v=0;
#if 1
  printf(" signal table_st : array4k := (");
  for(int i=0;table_st[i];i++) {
    if (table_st[i]=='h') {
      int n=strtoll(&table_st[i+1],NULL,16);
      i+=3;
      i+=8;
      v=strtoll(&table_st[i],NULL,16);
      int f=1;
      while(ofs<n) {
	if (!(ofs&0xf)) printf("\n");
	if (!first) { printf(", "); }
  ...
  printf(" signal table_p_t : array4k := (");
  first=1; ofs=0;
  for(int i=0;table_p_t[i];i++) {
    if (table_p_t[i]=='h') {
      int n=strtoll(&table_p_t[i+1],NULL,16);
      i+=3;
      i+=8;
      v=strtoll(&table_p_t[i],NULL,16);
      int f=1;
      while(ofs<n) {
	if (!(ofs&0xf)) printf("\n");
	if (!first) { printf(", "); }
  ...
  printf(" signal table_ps : array4k := (");
  first=1; ofs=0;
  for(int i=0;table_ps[i];i++) {
    if (table_ps[i]=='h') {
      int n=strtoll(&table_ps[i+1],NULL,16);
      i+=3;
      i+=8;
      v=strtoll(&table_ps[i],NULL,16);
      int f=1;
      while(ofs<n) {
	if (!(ofs&0xf)) printf("\n");
	if (!first) { printf(", "); }
  ...
  first=1;
  for(int i=0;table_pst[i];i++) {
    if (table_pst[i]=='h') {
      if (!first) printf("els"); first=0;
      printf("if sawtooth < x\"%c%c%c\" then ",
	     table_pst[i+1],table_pst[i+2],table_pst[i+3]
	     );
      i+=3;
      i+=8;
      printf("  pst_out <= x\"%c%c\";\n",
	     table_pst[i+0],table_pst[i+1]);
