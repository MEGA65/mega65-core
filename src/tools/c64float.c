...
test_case test_cases[] = { { .f = { { { 0x00, 0x12, 0x34, 0x56, 0x78 } } }, 0.0 },
  { .f = { { { 0x01, 0x00, 0x00, 0x00, 0x00 } } }, 2.93873588E-39 }, { .f = { { { 0x81, 0x00, 0x00, 0x00, 0x00 } } }, 1.0 },
  { .f = { { { 0x81, 0x80, 0x00, 0x00, 0x00 } } }, -1.0 }, { .f = { { { 0x80, 0x00, 0x00, 0x00, 0x00 } } }, 0.5 },
  { .f = { { { 0xFF, 0x7F, 0xFF, 0xFF, 0xFF } } }, 1.70141183e+38 },
  { .f = { { { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF } } }, -1.70141183e+38 },
  { .f = { { { 0x98, 0x35, 0x44, 0x7A, 0x00 } } }, 11879546.0 }, { .f = { { { 0x81, 0x40, 0x0, 0x0, 0x0 } } }, 1.5 },
  { .f = { { { 0x82, 0x40, 0x0, 0x0, 0x0 } } }, 3 }, { .f = { { { 0x82, 0x49, 0x0F, 0xDA, 0xA1 } } }, 3.14159265 }, // PI
...
int string_to_c64float(char *s, c64float *f)
{
  ...
  exp_total += extra_exp;
  if (decimal_places_plus_one)
    exp_total -= decimal_places_plus_one - 1;
  while (exp_total > 0) {
  ...
  if (f->f.exp < 0x81)
    f->f.exp--; // skip exponent = 0x80 for zero
...
int main(int argc, char **argv)
{
  ...
  for (int i = 0; test_cases[i].f.f.exp || test_cases[i].f.f.man0 || test_cases[i].d; i++) {
    ...
    if (err >= 0.000000003) {
      printf("ERROR converting from C64 floating point format to double: Result was %.9g, but should have been %.9g "
             "(difference = %.9g, error fraction=%.9g)\n",
