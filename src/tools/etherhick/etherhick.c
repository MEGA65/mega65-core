...
// that disables etherhick until next boot, so that no switch fiddling is required.
...
int usage()
{
  printf("usage:  etherhick <run|hickup> <IP address> <programme>\n");
  printf("        etherhick push <IP address> <file> <28-bit address (hex)>\n");
  exit(1);
}
...
int main(int argc, char**argv)
{
   ...
   if (runmode==1) {
     ...
     printf("Push mode -- leaving C65GS in etherhick.\n");
   }
