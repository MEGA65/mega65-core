...
int main(int argc,char **argv)
{
  ...
  do {
    ...
    if(!f) {
      retVal=-1; fprintf(stderr,"Could not open file '%s'\n",argv[1]);
      break;
    }

    raw_size=fread(raw,1,MAX_RAW_SIZE,f);
    if (raw_size<1) {
      retVal=-1; fprintf(stderr,"Couldd not read contents of input file.\n");
      break;
    }
    fclose(f);
    ...
    if (!o) {
      retVal=-1; fprintf(stderr,"ERROR: Could not open output file '%s'\n",argv[2]);
      break;
    }
    ...
    o=fopen(argv[2],"r");
    if (!o) {
      retVal=-1; fprintf(stderr,"ERROR: Could not open output file '%s' for verification\n",argv[2]);
      break;
    }
