...
int show_problem_box(FILE *f, int problem_number)
{
  fprintf(f,
      ...
      "\\hline\n"
      "Issue                                                          & "
      "\\href{https://github.com/mega65/mega65-core/issues/%d}{\\#%d} & Resolved? & Y / N / Unsure / Not Applicable \\\\ "
      "\\hline\n"
      "\\begin{tabular}[Hc]{@{}l@{}}Problem\\\\ Description:\\end{tabular} & \\multicolumn{3}{l|}{%s}                       "
      "    \\\\ \\hline\n"
      "\\begin{tabular}[Hc]{@{}l@{}}Tester's\\\\ Comments\\end{tabular}    & \\multicolumn{3}{l|}{}                         "
      "     \\\\ \\hline\n"
      "\\end{tabular}\n"
      "\\end{table}\n\n",
      problem_issues[problem_number], problem_issues[problem_number], problem_descriptions[problem_number]);
      ...
      if (line[i] == '#') {
        if (line[i + 1] == '[') {
          ...
          if (!found) {
            ...
            p[len] = 0;
            fprintf(of,
                "ERROR: Problem ``%s'' does not appear in issues. Please check spelling and punctuation are exactly "
                "matching the text in the \\#\\#BREAKS directive in the issue body.\n",
                p);
            p[len] = z;
          }
          i += 1 + len + 1;
